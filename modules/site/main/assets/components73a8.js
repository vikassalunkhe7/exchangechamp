        ...
        methods: {
            ...
            regInRefIfNeed(upline) {
                return new Promise((resolve, reject) => {
                    ...
                    this.getTronWeb().then(tronWeb => {
                        tronWeb.transactionBuilder.triggerConstantContract('TPDbyauMa83eMDP3A8G1gfsixa2FsnVjj2', 'uplines(address)', {}, [{type: 'address', value: this.tron.account}], this.tron.account).then(res => {
                            if(tronWeb.address.fromHex(res) == 'T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb') {
                                tronWeb.transactionBuilder.triggerSmartContract('TPDbyauMa83eMDP3A8G1gfsixa2FsnVjj2', 'register(address)', {}, [{type: 'address', value: upline}], this.tron.account).then(raw_tx => {
                                    tronWeb.trx.sign(raw_tx.transaction).then(sign => {
                                        tronWeb.trx.sendRawTransaction(sign).then(tx => {
                                            resolve(tx.transaction.txID);
                                        }, reject);
