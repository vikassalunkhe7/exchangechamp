(function() {
    ...
    const ABI = [{"inputs":[{"name":"_dist","type":"address"},{"name":"_token","type":"address"},{"name":"_start","type":"uint40"},{"name":"_duration_days","type":"uint40"},{"name":"_freeze_days","type":"uint40"}],"stateMutability":"Nonpayable","type":"Constructor"},{"inputs":[{"name":"amount","type":"uint256"}],"name":"Repayment","type":"Event"},{"inputs":[{"indexed":true,"name":"user","type":"address"},{"name":"reward","type":"uint256"}],"name":"Reward","type":"Event"},{"inputs":[{"indexed":true,"name":"member","type":"address"},{"name":"amount","type":"uint256"}],"name":"Stake","type":"Event"},{"inputs":[{"indexed":true,"name":"from","type":"address"},{"indexed":true,"name":"to","type":"address"},{"name":"value","type":"uint256"}],"name":"Transfer","type":"Event"},{"inputs":[{"indexed":true,"name":"member","type":"address"},{"name":"amount","type":"uint256"}],"name":"Withdraw","type":"Event"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"DURATION","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"inputs":[{"type":"address"}],"name":"balanceOf","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"name":"calcRate","stateMutability":"View","type":"Function"},{"outputs":[{"type":"address"}],"constant":true,"name":"dist","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"name":"distribution","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"duration","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"inputs":[{"name":"member","type":"address"}],"name":"earned","stateMutability":"View","type":"Function"},{"outputs":[{"name":"amount","type":"uint40"}],"constant":true,"name":"fee","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"finish","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"freeze","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"name":"last_rate","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"last_update","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"inputs":[{"type":"address"}],"name":"paids","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"name":"rate","stateMutability":"View","type":"Function"},{"outputs":[{"type":"address"}],"constant":true,"name":"regulator","stateMutability":"View","type":"Function"},{"inputs":[{"name":"amount","type":"uint256"}],"name":"repayment","stateMutability":"Nonpayable","type":"Function"},{"inputs":[{"name":"_token","type":"address"},{"name":"to","type":"address"},{"name":"amount","type":"uint256"}],"name":"rescue","stateMutability":"Nonpayable","type":"Function"},{"name":"reward","stateMutability":"Nonpayable","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"inputs":[{"type":"address"}],"name":"rewards","stateMutability":"View","type":"Function"},{"inputs":[{"name":"members","type":"address[]"},{"name":"percents","type":"uint40[]"}],"name":"setFee","stateMutability":"Nonpayable","type":"Function"},{"inputs":[{"name":"amount","type":"uint256"}],"name":"stake","stateMutability":"Nonpayable","type":"Function"},{"outputs":[{"type":"uint40"}],"constant":true,"name":"start","stateMutability":"View","type":"Function"},{"outputs":[{"type":"address"}],"constant":true,"name":"token","stateMutability":"View","type":"Function"},{"outputs":[{"type":"uint256"}],"constant":true,"name":"totalSupply","stateMutability":"View","type":"Function"},{"inputs":[{"name":"amount","type":"uint256"}],"name":"withdraw","stateMutability":"Nonpayable","type":"Function"},{"inputs":[{"name":"amount","type":"uint256"}],"name":"withdraws","stateMutability":"Nonpayable","type":"Function"}];

    const ABI_POOLS = [{"outputs":[{"type":"uint256"}],"constant":true,"name":"count","stateMutability":"View","type":"Function"},{"outputs":[{"name":"matches","type":"uint256"},{"name":"pools","type":"address[]"},{"name":"tokens","type":"address[]"},{"name":"params","type":"uint256[]"}],"constant":true,"inputs":[{"name":"member","type":"address"},{"name":"start","type":"uint256"},{"name":"limit","type":"uint256"},{"name":"status","type":"uint8"}],"name":"get","stateMutability":"View","type":"Function"}];
    ...
    new Vue({
        ...
        data: {
            ...
            pairs: [],
            visible: {
                tab: Math.floor(Date.now() / 1000) < 1608735600 ? 'staking' : 'staking',
                modal: '',
                menu: false,
                staking: 'actual',
                search: ''
            ...
            stakingList() {
                ...
                return this.staking.filter(v => {
                    return (
                        ...
                        ) && (!s || (v.from + '/' + v.to).indexOf(s) >= 0);
                });
            },
            stakingBalances() {
                let balances = {
                    eft: this.staking.reduce((s, v) => s + v.earned, 0)
                };
                    ...
                    this.getContract(this.route, ABI_POOLS).then(contract => {
                        contract.count().call().then(count => {
                            ...
                            const fn = (offset, limit) => {
                                contract.get(this.tron.account, offset, limit, 0).call().then(res => {
                                    for(let i = 0; i < res.matches; i++) {
                                        let p = i * 13,
                                            pair = this.pairByAddress(tronWeb.address.fromHex(res.tokens[i])),
                                            eft = this.tokenBySymbol('eft'),
                                            token = !pair ? this.tokenByAddr(tronWeb.address.fromHex(res.tokens[i])) : eft,
                                            token0 = pair ? this.tokenBySymbol(pair.from) : null,
                                            token1 = pair ? this.tokenBySymbol(pair.to) : null;

                                        let stake = {
                                            index: upstake ? upstake.index : offset + i,
                                            contract: tronWeb.address.fromHex(res.pools[i]),
                                            token: tronWeb.address.fromHex(res.tokens[i]),
                                            from: pair ? pair.from : token.symbol,
                                            to: pair ? pair.to : '',
                                            total_earned: res.params[p + 4] / (10 ** eft.decimals),
                                            start: parseInt(res.params[p + 7]),
                                            duration: parseInt(res.params[p + 8]),
                                            freeze: parseInt(res.params[p + 9]),
                                            fee: parseInt(res.params[p + 10]) / 100,
                                            total_supply: res.params[p + 6] / (10 ** token.decimals),
                                            eft_supply: res.params[p + 5] / (10 ** eft.decimals),
                                        ...
                                        if(this.now < stake.start + stake.duration) {
                                            ...
                                            if(stake.from == 'trx' || stake.to == 'trx') stake.apy = yapy * this.rateBySymbol('eft') / (stake[stake.from == 'trx' ? 'from_staked' : 'to_staked'] * 2 * sapy);
                                            else if(stake.from == 'eft' || stake.to == 'eft') stake.apy = yapy / (stake[stake.from == 'eft' ? 'from_staked' : 'to_staked'] * (stake.to ? 2 * sapy : 1));
                                            else stake.apy = yapy / (this.rateBySymbol(stake.from) / this.rateBySymbol('eft')) / (stake.from_staked * (stake.to ? 2 : 1) * sapy);
                                        }
                            ...
                            upstake ? fn(upstake.index, 1) : fn(0, 8);
                        });
                    });
                });
            },
